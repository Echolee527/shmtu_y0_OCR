...
imagePath = current_dir+r'\data\3.jpg'
print(imagePath)
...
windows = np.array(keep,dtype=np.int)
boxes = nms(windows,0.31)
for (x, y, x2, y2) in boxes:
    print (x, y, x2, y2)
    cv2.rectangle(orig, (int(x),int(y)),(int(x2), int(y2)),(0, 255, 0), 1)
